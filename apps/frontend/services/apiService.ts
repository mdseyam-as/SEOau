...
class ApiService {
    ...
    // Knowledge Base
    async getKnowledgeBase(): Promise<{ files: Array<{ id: string; fileName: string; fileType: string; fileSize: number; createdAt: string; chunkCount?: number; hasEmbeddings?: boolean; embeddingModel?: string | null; embeddingStatus?: string }> }> {
        return this.request('/knowledge-base');
    }

    async uploadKnowledgeBaseFile(file: File): Promise<{ id: string; fileName: string; fileType: string; fileSize: number; createdAt: string; chunkCount?: number; hasEmbeddings?: boolean; embeddingModel?: string | null; embeddingStatus?: string }> {
    ...
    async reindexKnowledgeBaseFile(id: string): Promise<{ file: { id: string; fileName: string; fileType: string; fileSize: number; createdAt: string; chunkCount?: number; hasEmbeddings?: boolean; embeddingModel?: string | null; embeddingStatus?: string } }> {
        return this.request(`/knowledge-base/${id}/reindex`, { method: 'POST' });
    }

    async searchKnowledgeBase(query: string, topK = 5): Promise<{ results: Array<{ chunkId: string; chunkIndex: number; file: { id: string; fileName: string; fileType?: string }; similarity: number; matchType: string; snippet: string }> }> {
    ...
    // Background Tasks
    async getTasks(): Promise<{ tasks: BackgroundTaskListItem[] }> {
