...
const LEGACY_BACKEND_STORAGE_KEYS = [
  'seo_gen_users_tg',
  'seo_gen_settings',
  'seo_gen_plans',
  'seo_gen_models'
...
let cachedModels: ModelConfig[] = [...DEFAULT_MODELS_LIST];
let cachedPlans: SubscriptionPlan[] = [...DEFAULT_PLANS];
let cachedUsers: User[] = [];

const clearLegacyBackendStorage = () => {
  if (typeof window === 'undefined' || !window.localStorage) return;

  try {
    LEGACY_BACKEND_STORAGE_KEYS.forEach((key) => window.localStorage.removeItem(key));
  } catch (error) {
    console.warn('Failed to clear legacy backend localStorage keys:', error);
  }
};
...
export const authService = {
  ...
  getPlans: (): SubscriptionPlan[] => {
    clearLegacyBackendStorage();
    let plans: SubscriptionPlan[] = [...cachedPlans];
  ...
  savePlans: (plans: SubscriptionPlan[]) => {
    cachedPlans = plans.length > 0 ? [...plans] : [...DEFAULT_PLANS];
    clearLegacyBackendStorage();
  },
