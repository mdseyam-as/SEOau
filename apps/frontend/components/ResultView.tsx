...
import {
  ...
  Download, FileDown, Wand2, Bot, Database
} from 'lucide-react';
...
export const ResultView: React.FC<ResultViewProps> = ({
  ...
  return (
    <div className="space-y-4 sm:space-y-5 lg:space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      {result.knowledgeBaseSources && result.knowledgeBaseSources.length > 0 && (
        <div className="app-dark-card p-4 sm:p-5 lg:p-6">
          <h3 className="font-bold text-sm sm:text-base lg:text-lg mb-3 text-white flex items-center gap-2">
            <Database className="w-4 h-4 sm:w-5 sm:h-5 text-cyan-400" />
            Источники RAG
            <span className="ml-auto text-xs font-normal bg-cyan-500/10 text-cyan-200 px-2 py-1 rounded-lg">
              {result.knowledgeBaseSources.length}
            </span>
          </h3>
          <div className="grid gap-3 md:grid-cols-2">
            {result.knowledgeBaseSources.slice(0, 6).map((source, index) => (
              <div key={source.chunkId || `${source.file.id}-${index}`} className="rounded-xl border border-white/10 bg-white/[0.03] p-3">
                <div className="flex items-center gap-2 text-xs text-slate-400">
                  <span className="font-bold text-white truncate">{source.file.fileName}</span>
                  <span className="ml-auto rounded bg-white/10 px-2 py-0.5 text-[11px] text-cyan-200">
