...
function buildRagSearchQuery(config, mainKeywords, topKeywords) {
    return [
        config.topic,
        mainKeywords ? `Main keywords: ${mainKeywords}` : '',
        config.lsiKeywords ? `LSI keywords: ${config.lsiKeywords}` : '',
        topKeywords ? `Keyword plan: ${topKeywords}` : '',
        config.targetCountry ? `Target region: ${config.targetCountry}` : '',
        config.websiteName ? `Website: ${config.websiteName}` : ''
    ].filter(Boolean).join('\n');
}

function buildKnowledgeBasePromptBlock(context) {
    if (!context) return '';

    return `
---
PRIVATE KNOWLEDGE BASE RAG CONTEXT:
Use these retrieved excerpts as source-of-truth when they are relevant to the user's topic. Prefer them over generic assumptions. Keep facts, names, prices, constraints, locations, and tone consistent with the sources. If the sources do not support a precise claim, write it as a recommendation or verification note.
...
router.post('/', validate(generateSchema), async (req, res) => {
    try {
        ...
        const ragMeta = {
            enabled: RAG_CONFIG.enabled !== false && sanitizedConfig.useKnowledgeBase !== false,
            used: false,
            query: '',
            sourceCount: 0,
        ...
        if (ragMeta.enabled) {
            try {
                const ragQuery = buildRagSearchQuery(sanitizedConfig, mainKeywords, topKeywords);
                const ragResult = await knowledgeBaseService.getGenerationContext(limitCheck.user.id, ragQuery, {
                    topK: sanitizedConfig.ragTopK || RAG_CONFIG.topK || 5,
                    maxContextLength: RAG_CONFIG.maxContextLength || 6000,
                    minSimilarity: RAG_CONFIG.similarityThreshold
                });

                knowledgeBaseContext = ragResult.context;
                knowledgeBaseSources = ragResult.sources;
                ragMeta.used = ragResult.used;
                ragMeta.query = ragQuery;
                ragMeta.sourceCount = ragResult.sources.length;
                ragMeta.embeddingModel = ragResult.embeddingModel;
            } catch (error) {
                ragMeta.error = error.message;
                console.error('[RAG] Knowledge base retrieval failed:', error.message);
            }
        }

        const knowledgeBasePromptBlock = buildKnowledgeBasePromptBlock(knowledgeBaseContext);
        console.log('>>> RAG RETRIEVAL:', {
            enabled: ragMeta.enabled,
            used: ragMeta.used,
            sources: ragMeta.sourceCount,
            error: ragMeta.error
        });
...
"""`
        ...
        const promptHasKnowledgeBasePlaceholder = prompt.includes('{{knowledgeBaseContext}}');
        ...
        const replacements = {
            ...
            '{{knowledgeBaseContext}}': knowledgeBasePromptBlock
        };
        ...
        if (knowledgeBasePromptBlock && !promptHasKnowledgeBasePlaceholder) {
...
${knowledgeBaseContext || 'No private knowledge base excerpts were retrieved.'}
${exampleInstruction}
            `.trim();
        ...
        result.knowledgeBaseSources = knowledgeBaseSources;
        result._rag = {
            ...ragMeta,
            sourceCount: knowledgeBaseSources.length
        };
