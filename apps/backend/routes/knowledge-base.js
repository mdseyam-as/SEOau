...
router.post('/upload', upload.single('file'), async (req, res) => {
  try {
    ...
    // Загружаем файл и индексируем чанки для RAG
    const result = await knowledgeBaseService.uploadFile(user.id, req.file);
    const fileDto = knowledgeBaseService.toFileDto(result);
 ...
 * Переиндексация файла: пересоздаёт чанки и embeddings.
 */
router.post('/:id/reindex', async (req, res) => {
  try {
    ...
    const file = await knowledgeBaseService.reindexFile(user.id, req.params.id);

    res.json({ file });
  } catch (error) {
    console.error('Knowledge base reindex error:', error);
    res.status(500).json({ error: error.message || 'Failed to reindex file' });
  }
});
...
router.post('/search', async (req, res) => {
  try {
    ...
    const publicResults = results.map(({ content, ...result }) => result);
