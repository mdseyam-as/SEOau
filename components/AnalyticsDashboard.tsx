            ...
            <div className="flex items-center justify-between mb-6 relative z-10">
               ...
               <div className="flex gap-4 text-[10px] md:text-xs">
                  <div className="flex items-center gap-1.5">
                     <div className="w-2 h-2 rounded-full bg-brand-blue shadow-[0_0_8px_rgba(56,189,248,0.6)]"></div>
                     <span className="text-slate-300">Частота (Excel)</span>
                  </div>
                  <div className="flex items-center gap-1.5">
                     <div className="w-2 h-2 rounded-full bg-brand-green shadow-[0_0_8px_rgba(0,220,130,0.6)]"></div>
                     <span className="text-slate-300">Плотность (Текст)</span>
                  </div>
               </div>
            </div>

            <div className="flex-1 flex items-end gap-1 md:gap-3 min-h-[180px] pb-2 border-b border-white/10 relative z-10">
            ...
            <div>
               ...
               <div className="mb-8">
                  <div className="flex justify-between items-end mb-3">
                     <span className="text-sm text-slate-300">Оценка релевантности</span>
                     <span className={`text-2xl font-bold ${relevanceScore >= 80 ? 'text-brand-blue drop-shadow-glow' : relevanceScore >= 50 ? 'text-yellow-400' : 'text-red-400'}`}>
                        {relevanceScore}/100
                     </span>
                  </div>
                  ...
                  {!isPerfectScore && hasMissingKeys && onOptimizeRelevance && (
                     <button
                        ...
                        className={`
                        w-full mt-6 flex items-center justify-center gap-2 py-2.5 rounded-xl text-xs font-bold transition-all shadow-lg
                        ${canOptimize
               ...
               <div className="pt-6 border-t border-white/10">
                  <div className="flex items-center gap-2 mb-2 text-slate-300 text-sm">
                     <FileText className="w-4 h-4 text-brand-green" /> Количество слов
                  </div>
                  <div className="text-3xl font-bold text-white tracking-tight drop-shadow-sm">{wordCount}</div>
                  {/* Visual indicator for word count aim */}
                  <div className="flex gap-1.5 mt-3">
                     {[1, 2, 3, 4, 5].map(i => (
                        <div key={i} className={`h-1.5 flex-1 rounded-full transition-colors ${wordCount > i * 500 ? 'bg-brand-green shadow-[0_0_5px_rgba(0,220,130,0.5)]' : 'bg-white/10'}`}></div>
                     ))}
                  </div>
               </div>
            </div>
